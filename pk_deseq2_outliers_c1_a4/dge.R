...
run_pipeline = function(condition1, condition2){
  ...
  ego.data.frame$GeneRatio = sapply(strsplit(ego.data.frame$GeneRatio, split = "/"),
         function(x) as.numeric(x[1]) / as.numeric(x[2]))
  ego.data.frame = ego.data.frame[order(ego.data.frame$GeneRatio, decreasing = T), ]
  write.csv(ego.data.frame, 'over_representation_analysis/GO/ora_pathways.csv')
  ...
  ego.data.frame$GeneRatio = sapply(strsplit(ego.data.frame$GeneRatio, split = "/"),
                                    function(x) as.numeric(x[1]) / as.numeric(x[2]))
  ego.data.frame = ego.data.frame[order(ego.data.frame$GeneRatio, decreasing = T), ]
  write.csv(ego.data.frame, 'over_representation_analysis/GO_mf/ora_pathways.csv')
  ...
  ego.data.frame = data.frame(ekegg)
  ego.data.frame$GeneRatio = sapply(strsplit(ego.data.frame$GeneRatio, split = "/"),
                                    function(x) as.numeric(x[1]) / as.numeric(x[2]))
  ego.data.frame = ego.data.frame[order(ego.data.frame$GeneRatio, decreasing = T), ]
  write.csv(ego.data.frame, 'over_representation_analysis/KEGG/ora_pathways.csv')
  ...
  write.csv(gseaGO_results[order(gseaGO_results$NES, decreasing = T),], 'clusterProfiler_gsea/GO/enriched_pathways.csv')
